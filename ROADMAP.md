...
- Requires injecting login session cookies
...
**Chosen tool: [Scrapling](https://github.com/D4Vinci/Scrapling)** (evaluated in #2), not raw  
Playwright:

- `StealthyFetcher`/`DynamicFetcher` accept `cookies=` and `user_data_dir=` — drop in exported  
FB login cookies, page loads already-authenticated, no login-flow automation needed
- Fingerprint spoofing + `network_idle` handle FB's heavier JS/lazy-load better than raw Playwright
- `capture_xhr` can grab the group's internal GraphQL/JSON responses instead of parsing rendered  
HTML — more robust to FB's frequent markup changes
- **Caveat:** Scrapling's stealth targets Cloudflare/Turnstile-style bot walls, not Meta's own  
detection (fingerprinting, login-anomaly checks, checkpoint challenges). Cookie injection  
skips the login flow, but a scraping pattern (many pages, same IP) can still trigger FB rate  
limiting — same risk as plain Playwright, just better stealth/session ergonomics
- Instagram was *not* evaluated for Scrapling — Phase 4's Graph API approach is more stable and  
ToS-compliant than any scrape, no reason to replace it
- Next step: prototype `src/scrapers/facebook.py` using  
`StealthyFetcher.fetch(group_url, cookies=session_cookies)` against one target group to confirm  
it loads posts without hitting a login wall
